document.addEventListener('DOMContentLoaded', function() {
  ...
  document.addEventListener('click', function(e) {
    if (e.target.classList.contains('floor-plane')) {
      ...
      // 布局图路径
      const layoutImgPath = `./floor-layouts/building${buildingNum}-floor${floorNum}.jpg`;
      ...
      // 只加1次手柄，且放在innerHTML之后（避免被覆盖）
      let resizeHandle = layoutPanel.querySelector('.resize-handle');
      if (!resizeHandle) {
      ...
      layoutPanel.addEventListener('mousemove', function(e) {
        ...
        // 边缘检测范围（10px内）
        const isLeftEdge = e.clientX >= rect.left && e.clientX <= rect.left + 10;
        const isBottomEdge = e.clientY >= rect.bottom - 10 && e.clientY <= rect.bottom;

        // 根据位置切换光标和缩放类型
        if (isLeftEdge && isBottomEdge) {
          layoutPanel.className = 'floor-layout-panel resizable-left-corner';
          resizeType = 'left-corner';
        } else if (isLeftEdge) {
          layoutPanel.className = 'floor-layout-panel resizable-left';
          resizeType = 'left';
      ...
      document.addEventListener('mousemove', function(e) {
        ...
        } else if (resizeType === 'left-corner') {
          newWidth = startWidth + (startX - e.clientX);
          newHeight = startHeight + (e.clientY - startY);
        }

        // 限制最小尺寸
        if (newWidth >= 250) layoutPanel.style.width = `${newWidth}px`;
      ...
      // 关闭按钮
      layoutPanel.querySelector('.close-layout-btn').addEventListener('click', function() {
        layoutPanel.style.display = 'none';
      });

      // 重置尺寸按钮
      layoutPanel.querySelector('.reset-size-btn').addEventListener('click', function() {
        layoutPanel.style.width = '350px';
        layoutPanel.style.height = '400px';
      });
    } // 【核心修正】补上if (e.target.classList.contains('floor-plane')) 的闭合括号
  }); // 【核心修正】补上document.addEventListener('click') 的闭合括号
